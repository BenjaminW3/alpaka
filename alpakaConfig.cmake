...
#-------------------------------------------------------------------------------
# Options.
SET(ALPAKA_ACC_CPU_B_SEQ_T_COROUTINES_ENABLE_DEFAULT ON)
...
# Coroutines are only supported by some compilers.
IF(NOT (MSVC OR (CMAKE_CXX_COMPILER_ID MATCHES "Clang" AND CMAKE_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL 5.0)))
    SET(ALPAKA_ACC_CPU_B_SEQ_T_COROUTINES_ENABLE_DEFAULT OFF)
ENDIF()
...
OPTION(ALPAKA_ACC_CPU_B_SEQ_T_COROUTINES_ENABLE "Enable the coroutines CPU block thread back-end" ${ALPAKA_ACC_CPU_B_SEQ_T_COROUTINES_ENABLE_DEFAULT})
...
IF((ALPAKA_ACC_GPU_CUDA_ONLY_MODE OR ALPAKA_ACC_GPU_HIP_ONLY_MODE)
   AND
    (ALPAKA_ACC_CPU_B_SEQ_T_COROUTINES_ENABLE OR
...
# Coroutines are not supported in combination with CUDA
IF(ALPAKA_ACC_CPU_B_SEQ_T_COROUTINES_ENABLE AND ALPAKA_ACC_GPU_CUDA_ENABLE)
    MESSAGE(FATAL_ERROR "CUDA and Coroutines can not be enabled both at the same time.")
ENDIF()
...
IF(MSVC)
    ...
    IF(ALPAKA_ACC_CPU_B_SEQ_T_COROUTINES_ENABLE)
        LIST(APPEND _ALPAKA_COMPILE_OPTIONS_PUBLIC "/await")
        LIST(APPEND _ALPAKA_COMPILE_OPTIONS_PUBLIC "/std:c++latest")
    ENDIF()
ELSE()
    ...
    ELSEIF(CMAKE_CXX_COMPILER_ID MATCHES "Clang")
        LIST(APPEND _ALPAKA_COMPILE_OPTIONS_PUBLIC "-ftemplate-depth=512")
        IF(ALPAKA_ACC_CPU_B_SEQ_T_COROUTINES_ENABLE)
            LIST(APPEND _ALPAKA_COMPILE_OPTIONS_PUBLIC "-std=c++17")
            LIST(APPEND _ALPAKA_COMPILE_OPTIONS_PUBLIC "-fcoroutines-ts")
        ENDIF()
    ENDIF()
ENDIF()
...
IF(ALPAKA_ACC_CPU_B_SEQ_T_COROUTINES_ENABLE)
    LIST(APPEND _ALPAKA_COMPILE_DEFINITIONS_PUBLIC "ALPAKA_ACC_CPU_B_SEQ_T_COROUTINES_ENABLED")
    MESSAGE(STATUS ALPAKA_ACC_CPU_B_SEQ_T_COROUTINES_ENABLED)
ENDIF()
